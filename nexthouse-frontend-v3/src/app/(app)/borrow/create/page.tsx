'use client';
// src/app/(app)/borrow/create/page.tsx
  ...
  const onSubmit = async (d: Form) => {
    ...
    try {
      await borrowApi.create({
        ...
        itemType: d.itemType, durationDays: d.durationDays,
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="px-4 py-6 space-y-5 max-w-lg mx-auto pb-20">
        <div>
          ...
          <input {...register('title')} type="text" placeholder="e.g. Drill machine for 2 days" className={`input ${errors.title ? 'input-error' : ''}`}/>
        ...
        <div>
          ...
          <div className="grid grid-cols-4 gap-2">
            {ITEM_TYPES.map(t => (
              <button key={t} type="button" onClick={() => setValue('itemType', t)}
                className={`py-2 rounded-xl border-2 text-xs font-semibold transition ${selType === t ? 'border-primary-500 bg-primary-50 text-primary-700' : 'border-gray-200 text-gray-600'}`}>
                {t}
              </button>
            ))}
          </div>
        ...
        <div>
          ...
          <textarea {...register('description')} rows={3} placeholder="Why do you need it? Any specific requirements?" className="input resize-none"/>
        </div>
        <div>
          <label className="label">Duration needed (days)</label>
          <input {...register('durationDays', { valueAsNumber: true })} type="number" min={1} max={30} placeholder="1-30 days" className="input"/>
        </div>
        <button type="submit" disabled={loading} className="btn-primary w-full py-3.5">
          {loading ? <><Loader2 size={18} className="animate-spin"/>Posting…</> : <><Package size={18}/>Post Request</>}
        </button>
      </form>
    </div>
  );
