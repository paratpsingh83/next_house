            ...
            <Link key={room.id} href={`/chat/${room.id}`}>
              <div className="flex items-center gap-3 px-2 py-3 rounded-xl hover:bg-gray-100 transition">
                <div className="w-12 h-12 rounded-full bg-primary-100 flex items-center justify-center flex-shrink-0 relative overflow-hidden">
                  {room.avatarUrl?<img src={room.avatarUrl} className="w-full h-full object-cover" alt=""/>:
                    room.roomType==='GROUP'?<Users size={22} className="text-primary-500"/>:<span className="text-primary-600 font-bold text-lg">{(room.title??'?')[0].toUpperCase()}</span>}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between"><span className="font-semibold text-gray-900 truncate">{room.title??'Direct Chat'}</span>
                ...
                {(room.unreadCount??0)>0&&<span className="min-w-5 h-5 bg-primary-500 text-white text-xs rounded-full flex items-center justify-center px-1">{room.unreadCount}</span>}
              </div>
            </Link>
          ))}
        </div>
