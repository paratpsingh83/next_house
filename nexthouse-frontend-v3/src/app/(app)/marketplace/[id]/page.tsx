...
import {
  ...
  MessageCircle, Heart, CheckCircle, XCircle,
  Trash2, Edit3, Package, Star, ShoppingBag
} from 'lucide-react';
...
export default function MarketplaceItemPage() {
  ...
  const handleMarkSold = async () => {
    ...
    try {
      await marketplaceApi.markSold(itemId);
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white px-4 pt-4 pb-5 border-b border-gray-100">
        ...
        <div className="flex items-center gap-2 mb-2 flex-wrap">
          {item.category && (
            <span className="badge bg-gray-100 text-gray-600 gap-1 text-xs">
          ...
          {item.negotiable && (
            <span className="badge bg-blue-50 text-blue-600 text-xs">Negotiable</span>
          )}
        </div>
          ...
          <div className="card p-4 flex items-center gap-3 hover:shadow-md transition">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-gray-400">
                Trust score: {item.seller.trustScore ?? 0}
                {item.seller.online ? ' · Online now' : ''}
              </p>
            </div>
